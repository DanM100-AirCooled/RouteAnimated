...
function initMap() {
  ...
  map = L.map('map').setView([29.73265, -96.7282], 8); // Centered between Katy and Luling
  ...
  // Define the waypoints: Buc-ee's Katy, Bellville, La Grange, and Buc-ee's Luling
  const waypoints = [
    L.latLng(29.7849, -95.8101), // Buc-ee's Katy, TX
    L.latLng(29.9507, -96.2578), // Bellville, TX
    L.latLng(29.9043, -96.8764), // La Grange, TX
    L.latLng(29.6804, -97.6463)  // Buc-ee's Luling, TX
  ];
...
function animateMarkerAlongRoute(waypoints) {
  ...
  function moveMarker() {
    if (index < waypoints.length) {
      ...
      marker.bindPopup("Arrived at Buc-ee's Luling, TX").openPopup();
    }
  }
...
function addRouteTimeline() {
  ...
  const estimatedTravelTimeMinutes = 180; // Example travel time of 180 minutes
