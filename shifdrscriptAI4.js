...
function initMap() {
  ...
  // Define the waypoints: Sealy, Cat Spring, Restroom Stop, New Ulm, Photo Shoot, Industry, Bellville, and back to Sealy
  const waypoints = [
    L.latLng(29.7805, -96.157),  // Sealy, TX
    L.latLng(29.8525, -96.3169), // Cat Spring, TX
    L.latLng(29.8805, -96.4169), // Restroom Stop
    L.latLng(29.8941, -96.5072), // New Ulm, TX
    L.latLng(29.9400, -96.5500), // Photo Shoot
  ...
  const control = L.Routing.control({
    ...
    createMarker: function(i, waypoint) {
      ...
      const popup = L.marker(waypoint.latLng).bindPopup(labels[i]).openPopup();
      return popup;
    },
  ...
  marker = L.marker(waypoints[0]).addTo(map).bindPopup('Sealy, TX').openPopup();
...
function animateMarkerAlongRoute(routeCoordinates) {
  let index = 0;
  const updateInterval = 50; // Speed up the animation

  function moveMarker() {
    if (index < routeCoordinates.length) {
      const newLatLng = routeCoordinates[index];
...
function addRouteTimeline() {
  ...
  const estimatedTravelTimeMinutes = 150; // Example travel time of 150 minutes
